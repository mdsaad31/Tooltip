    ...
    <div className={`max-w-6xl mx-auto p-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              <h3 className="text-white text-lg font-semibold mb-3">
                {qrType.charAt(0).toUpperCase() + qrType.slice(1)} Settings
              </h3>
              {renderInputFields()}
            </div>
            ...
            {/* Advanced Settings */}
            <GlassCard size="sm">
              <div className="text-white/80 text-sm font-medium mb-3">Advanced Settings</div>
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-white/70 text-xs mb-1">Error Correction</label>
                    <select
                      className="w-full backdrop-blur-md bg-white/10 border border-white/20 rounded px-2 py-1 text-white text-sm"
                      value={config.errorCorrectionLevel}
                      onChange={(e) => setConfig(prev => ({ ...prev, errorCorrectionLevel: e.target.value as ErrorCorrectionLevel }))}
                    >
                  ...
                  <div>
                    <label className="block text-white/70 text-xs mb-1">Size (px)</label>
                    <input
                      type="number"
                      min="128"
                      max="1024"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-white/70 text-xs mb-1">Foreground Color</label>
                    <div className="flex space-x-2">
                      <input
                        type="color"
                      ...
                      <input
                        type="text"
                        className="flex-1 backdrop-blur-md bg-white/10 border border-white/20 rounded px-2 py-1 text-white text-xs"
                        value={config.color.dark}
                        onChange={(e) => setConfig(prev => ({ 
                          ...prev, 
                          color: { ...prev.color, dark: e.target.value } 
                  ...
                  <div>
                    <label className="block text-white/70 text-xs mb-1">Background Color</label>
                    <div className="flex space-x-2">
                      <input
                        type="color"
                        className="w-8 h-8 rounded border border-white/20 bg-transparent"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-white/70 text-xs mb-1">Margin</label>
                    <input
                      type="number"
                      min="0"
                      max="10"
                  ...
                  <div>
                    <label className="block text-white/70 text-xs mb-1">Format</label>
                    <select
                      className="w-full backdrop-blur-md bg-white/10 border border-white/20 rounded px-2 py-1 text-white text-sm"
                      value={config.type}
                      onChange={(e) => setConfig(prev => ({ ...prev, type: e.target.value as 'image/png' | 'image/jpeg' }))}
          ...
          {/* QR Code Display */}
          <div className="space-y-4">
            <GlassCard size="md" className="text-center">
              {qrCodeDataURL ? (
                <div className="space-y-4">
                  <img 
                    src={qrCodeDataURL} 
                ...
                <div className="py-12 text-white/60">
                  {isGenerating ? (
                    <div className="space-y-3">
                    ...
                    <div className="space-y-3">
                      <QrCodeIcon className="w-16 h-16 mx-auto opacity-50" />
